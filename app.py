...
st.sidebar.markdown("---")
st.sidebar.markdown("**Справка по формату:** Программа автоматически очищает маркеры `E`, динамически выравнивает точность осей и делит строки на блоки.")
...
def nips_to_wgs84(nips_token, zone, square):
    try:
        ...
        # Динамически отсекаем или дополняем Northing, чтобы длина СТРОГО совпадала с Easting (5 знаков)
        northing_raw = nips_token[6:11]
        if len(northing_raw) < 5:
            northing_raw = northing_raw.ljust(5, '0')
        elif len(northing_raw) > 5:
            northing_raw = northing_raw[:5]
            
        mgrs_string = f"{zone}{square}{easting_raw}{northing_raw}"
        m = MGRS()
        lat, lon = m.toLatLon(mgrs_string.encode('utf-8'))
...
if raw_data:
    ...
    with col2:
        ...
        if not df_valid.empty:
            center_lat = df_valid.iloc[0]["Широта (Lat)"]
            center_lon = df_valid.iloc[0]["Долгота (Lon)"]
